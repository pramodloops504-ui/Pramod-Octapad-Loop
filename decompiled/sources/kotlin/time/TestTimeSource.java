...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m1689plusAssignLRDsOJo(long duration) {
        long longDelta = Duration.m1599toLongimpl(duration, getUnit());
        ...
        if ($i$f$isSaturated == 0) {
            ...
            if ((this.reading ^ longDelta) >= 0 && (this.reading ^ newReading) < 0) {
                m1688overflowLRDsOJo(duration);
            }
            this.reading = newReading;
            return;
        }
        long half = Duration.m1556divUwyO8pc(duration, 2);
        long $this$isSaturated$iv = Duration.m1599toLongimpl(half, getUnit());
        if (!((($this$isSaturated$iv - 1) | 1) == Long.MAX_VALUE)) {
            long readingBefore = this.reading;
            try {
                m1689plusAssignLRDsOJo(half);
                m1689plusAssignLRDsOJo(Duration.m1588minusLRDsOJo(duration, half));
                return;
    ...
    private final void m1688overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + DurationUnitKt.shortName(getUnit()) + " is advanced by " + ((Object) Duration.m1602toStringimpl(duration)) + '.');
    }
}
