...
public final class ResumeAwaitOnCompletion<T> extends JobNode {
    ...
    public void invoke(Throwable cause) {
        ...
        if (state instanceof CompletedExceptionally) {
            ...
            cancellableContinuationImpl.resumeWith(Result.m227constructorimpl(ResultKt.createFailure(((CompletedExceptionally) state).cause)));
        } else {
            CancellableContinuationImpl<T> cancellableContinuationImpl2 = this.continuation;
            Result.Companion companion2 = Result.INSTANCE;
            cancellableContinuationImpl2.resumeWith(Result.m227constructorimpl(JobSupportKt.unboxState(state)));
        }
    }
}
